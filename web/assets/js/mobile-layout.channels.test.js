...
test('channels 页顶部筛选控件不再写死桌面宽度', () => {
  ...
  assert.doesNotMatch(channelsHtml, /class="channel-page-hero"[\s\S]*class="channel-page-actions"/);
  assert.match(channelsHtml, /id="exportCsvBtn"[^>]*class="btn btn-secondary channel-filter-action-btn"/);
  assert.match(channelsHtml, /id="importCsvBtn"[^>]*class="btn btn-secondary channel-filter-action-btn"/);
  assert.match(channelsHtml, /data-action="show-add-modal"[^>]*class="btn btn-primary channel-filter-action-btn"/);
  ...
  assert.match(channelsHtml, /id="btn_filter"[^>]*class="btn btn-primary channel-filter-action-btn display-none"/);
});
...
test('channels 页手机端将渠道名称和模型放在同一行并把清空归入操作区', () => {
  ...
  assert.match(channelsHtml, /class="channel-filter-summary"[\s\S]*id="filterInfo"[\s\S]*id="exportCsvBtn"[\s\S]*id="importCsvBtn"[\s\S]*id="btn_sort"[\s\S]*id="clearSearchBtn"/);
  assert.match(channelsHtml, /class="channel-toolbar-actions"[\s\S]*channel-toolbar-group--create[\s\S]*show-add-modal[\s\S]*show-quick-add-modal[\s\S]*channel-toolbar-group--data[\s\S]*exportCsvBtn[\s\S]*importCsvBtn[\s\S]*btn_sort[\s\S]*channel-toolbar-group--view[\s\S]*channelGroupViewBtn[\s\S]*show-channel-group-manager[\s\S]*clearSearchBtn/);
  ...
  assert.match(channelsCss, /\.channel-search-control\s+\.filter-select\s*\{[\s\S]*?width:\s*100%;/);
  assert.doesNotMatch(channelsCss, /\.channel-search-control\s+#clearSearchBtn\s*\{/);
  assert.match(channelsCss, /\.channel-filter-summary\s+#clearSearchBtn\s*\{[\s\S]*?width:\s*auto;[\s\S]*?font-size:\s*0\.8rem;[\s\S]*?padding:\s*6px\s+8px;/);
  assert.match(channelsCss, /\.channel-filter-summary\s+\.channel-filter-action-btn\s*\{[\s\S]*?font-size:\s*0\.8rem;[\s\S]*?padding:\s*6px\s+8px;/);
});

test('channels 页超小屏顶部筛选保持一行两个选项', () => {
  assert.match(channelsCss, /@media\s*\(max-width:\s*480px\)\s*\{[\s\S]*?\.channels-filter-controls\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\);/);
  assert.match(channelsCss, /@media\s*\(max-width:\s*480px\)\s*\{[\s\S]*?\.channels-filter-controls\s+\.channel-search-filter-group,\s*[\r\n\s]*\.channels-filter-controls\s+\.channel-model-filter-group\s*\{[\s\S]*?grid-column:\s*1\s*\/\s*-1;/);
});

test('channels 页手机端头部和导入导出操作保持紧凑', () => {
  assert.doesNotMatch(channelsHtml, /channels\.description/);
  ...
  assert.match(channelsCss, /\.filter-bar\s*\{[\s\S]*?padding:\s*0\.85rem\s+0\.95rem;[\s\S]*?border-radius:\s*1\.25rem;/);
  assert.match(channelsCss, /\.channels-filter-controls\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*max-content\)\)\s+minmax\(220px,\s*1fr\)\s+minmax\(220px,\s*1\.15fr\);[\s\S]*?gap:\s*12px;/);
  assert.match(channelsCss, /\.channels-filter-controls\s+\.filter-group\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*64px\s+minmax\(0,\s*1fr\);[\s\S]*?align-items:\s*center;/);
  assert.match(channelsCss, /\.channels-filter-controls\s+\.filter-label\s*\{[\s\S]*?text-align:\s*right;[\s\S]*?font-size:\s*0\.78rem;[\s\S]*?font-weight:\s*600;/);
  assert.match(channelsCss, /\.channel-filter-summary\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s+auto;[\s\S]*?grid-column:\s*1\s*\/\s*-1;/);
  assert.match(channelsCss, /\.channel-filter-action-btn\s*\{[\s\S]*?padding:\s*7px\s+12px;[\s\S]*?font-size:\s*13px;/);
});

test('channels 页将数量、排序和筛选归到同一个移动端摘要行', () => {
  assert.match(channelsHtml, /class="channel-filter-summary"[\s\S]*id="filterInfo"[\s\S]*id="btn_sort"/);
  assert.match(channelsCss, /\.channel-toolbar-group\s*\{[\s\S]*?display:\s*inline-flex;[\s\S]*?gap:\s*8px;[\s\S]*?flex-wrap:\s*wrap;/);
  assert.match(channelsCss, /\.channel-toolbar-actions\s*\{[\s\S]*?display:\s*flex;[\s\S]*?gap:\s*8px;[\s\S]*?flex-wrap:\s*wrap;/);
  assert.match(channelsCss, /\.channel-toolbar-actions\s*>\s*\.btn,\s*[\r\n\s]*\.channel-toolbar-actions\s*>\s*\.channels-view-switch,\s*[\r\n\s]*\.channel-toolbar-actions\s*>\s*\.channel-toolbar-group\s*\{[\s\S]*?flex:\s*0\s+0\s+auto;/);
  assert.match(channelsCss, /\.channel-toolbar-group--data,\s*[\r\n\s]*\.channel-toolbar-group--view\s*\{[\s\S]*?border:\s*1px solid var\(--surface-border\);[\s\S]*?border-radius:\s*10px;[\s\S]*?background:\s*var\(--surface-bg-muted\);/);
  assert.match(channelsCss, /\.channels-view-switch\s*\{[\s\S]*?padding:\s*2px;[\s\S]*?border:\s*1px solid var\(--neutral-200\);[\s\S]*?border-radius:\s*8px;[\s\S]*?background:\s*var\(--surface-bg-muted\);/);
  assert.match(channelsCss, /\.channels-view-switch\s+\.btn,\s*[\r\n\s]*\.channel-toolbar-actions\s+\.btn-sm\s*\{[\s\S]*?min-height:\s*34px;[\s\S]*?padding:\s*6px\s+10px;[\s\S]*?font-size:\s*12px;/);
  assert.match(channelsCss, /\.channels-view-switch\s+\.btn\.active\s*\{[\s\S]*?background:\s*var\(--primary-500\);[\s\S]*?border-color:\s*var\(--primary-500\);[\s\S]*?color:\s*#fff;/);
});

test('channels 页去掉底部分页区域', () => {
  assert.doesNotMatch(channelsHtml, /logs-pagination-card/);
  assert.doesNotMatch(channelsHtml, /channels_current_page/);
  assert.doesNotMatch(channelsHtml, /channels_page_size/);
});
...
test('channels 页为手机卡片式表格预留移动端标签与样式', () => {
  ...
  assert.match(template, /class="ch-col-last-success[^"]*"[^>]*data-mobile-label="\{\{mobileLabelLastSuccess\}\}"/);
  assert.match(template, /class="ch-col-enabled"[^>]*data-mobile-label="\{\{mobileLabelEnabled\}\}"/);
  ...
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-cost\s*\{[^}]*order:\s*12;/);
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-last-success\s*\{[^}]*order:\s*13;/);
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-priority,\s*[\r\n\s]*\.channel-table\s+\.ch-col-cost,\s*[\r\n\s]*\.channel-table\s+\.ch-col-last-success,\s*[\r\n\s]*\.channel-table\s+\.ch-col-enabled\s*\{[\s\S]*?display:\s*flex;[\s\S]*?justify-content:\s*space-between;/);
  ...
  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?\.channel-table\s+tbody\s+tr\s*\{[\s\S]*?background:\s*var\(--surface-bg-strong\);/);
  assert.doesNotMatch(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?\.channel-table\s+tbody\s+tr\s*\{[\s\S]*?background:\s*rgba\(255,\s*255,\s*255,\s*0\.98\);/);
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-priority,\s*[\r\n\s]*\.channel-table\s+\.ch-col-duration,\s*[\r\n\s]*\.channel-table\s+\.ch-col-usage,\s*[\r\n\s]*\.channel-table\s+\.ch-col-cost,\s*[\r\n\s]*\.channel-table\s+\.ch-col-last-success,\s*[\r\n\s]*\.channel-table\s+\.ch-col-enabled\s*\{[\s\S]*?background:\s*var\(--surface-bg-muted\);[\s\S]*?border:\s*1px\s+solid\s+var\(--surface-border\);/);
  assert.doesNotMatch(channelsCss, /rgba\(248,\s*250,\s*252,\s*0\.9\)/);
...
test('channels 页手机卡片对空统计块做折叠', () => {
  ...
  assert.match(template, /class="ch-col-priority"/);
  assert.doesNotMatch(template, /priorityCellClass/);
  ...
  assert.match(template, /class="ch-col-last-success \{\{lastSuccessCellClass\}\}"/);

  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?\.channel-table\s+td\.ch-mobile-empty\s*\{[\s\S]*?display:\s*none;/);
  assert.doesNotMatch(channelsCss, /ch-mobile-span-full/);
});

test('channels 页手机卡片按用户要求重排关键状态行', () => {
  const templateMatch = channelsHtml.match(/<template id="tpl-channel-card">[\s\S]*?<\/template>/);
  assert.ok(templateMatch, '缺少渠道行模板');
  const template = templateMatch[0];
  ...
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-last-success\s*\{[^}]*order:\s*13;[\s\S]*?grid-column:\s*1\s*\/\s*-1;/);
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-last-success\s+\.ch-last-request-slot\s*\{[\s\S]*?margin-top:\s*0;[\s\S]*?margin-left:\s*auto;/);
  assert.match(channelsCss, /\.channel-table\s+\.ch-col-cost\s+\.cost-stack\s*\{[\s\S]*?flex-direction:\s*row;[\s\S]*?flex-wrap:\s*nowrap;[\s\S]*?white-space:\s*nowrap;/);
});

test('channels 页耗时和消耗列内部指标保持紧凑', () => {
  assert.match(channelsCss, /\.ch-timing\s*\{[\s\S]*?width:\s*max-content;[\s\S]*?max-width:\s*100%;[\s\S]*?margin:\s*0 auto;/);
  assert.match(channelsCss, /\.ch-usage-list\s*\{[\s\S]*?width:\s*max-content;[\s\S]*?max-width:\s*100%;[\s\S]*?margin:\s*0 auto;/);
  assert.match(channelsCss, /\.ch-timing-value,\s*[\r\n\s]*\.ch-usage-value\s*\{[\s\S]*?margin-left:\s*0;/);
});
...
test('channels 弹窗内联表为手机布局补齐类名、标签和关键重排', () => {
  ...
  assert.match(channelsHtml, /class="inline-url-col-exact[^"]*"[^>]*data-mobile-label="\{\{mobileLabelExactURL\}\}"/);
  ...
  assert.match(channelsUrlsScript, /setAttribute\('data-mobile-label', window\.t\('channels\.urlRequests'\)\)/);
...
test('channels 编辑弹窗为手机布局补齐结构化骨架和分组重排样式', () => {
  ...
  assert.match(channelsHtml, /class="form-group channel-editor-group channel-editor-group--primary"/);
  ...
  assert.match(channelsHtml, /class="channel-editor-primary-field channel-editor-primary-field--mode"/);
...
test('channels 编辑弹窗在手机端将基础字段、按钮条和卡片内容压成单行信息流', () => {
  ...
  assert.match(channelsCss, /\.channel-editor-inline-field\s*\{[\s\S]*?display:\s*grid;[\s\S]*?grid-template-columns:\s*auto\s+minmax\(0,\s*1fr\);[\s\S]*?align-items:\s*center;[\s\S]*?min-width:\s*0;/);
  assert.match(channelsCss, /\.channel-editor-scheduled-model-control\s*\{[\s\S]*?width:\s*100%;[\s\S]*?min-width:\s*0;[\s\S]*?max-width:\s*none;/);
  ...
  assert.match(channelsCss, /#channelModal\s+\.channel-editor-footer-fields\s+\.channel-editor-inline-field\s*>\s*\.form-input,[\s\S]*?#channelModal\s+\.channel-editor-footer-fields\s+\.filter-combobox\s*\{[\s\S]*?width:\s*100%\s*!important;[\s\S]*?min-width:\s*0\s*!important;[\s\S]*?max-width:\s*100%\s*!important;[\s\S]*?box-sizing:\s*border-box;/);
  ...
  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?#channelForm\.channel-editor-form\s*\{[\s\S]*?min-height:\s*0;[\s\S]*?padding-bottom:\s*0;/);
  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?#channelModal\s+\.channel-editor-footer\s*\{[\s\S]*?position:\s*static;[\s\S]*?left:\s*auto;[\s\S]*?right:\s*auto;[\s\S]*?bottom:\s*auto;[\s\S]*?z-index:\s*auto;[\s\S]*?padding:\s*10px\s+12px;[\s\S]*?border-radius:\s*14px;/);
  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?#channelModal\s+\.channel-editor-footer\s*\{[\s\S]*?background:\s*var\(--surface-bg-strong\);/);
  assert.doesNotMatch(channelsCss, /#channelModal\s+\.channel-editor-footer\s*\{[\s\S]*?background:\s*rgba\(255,\s*255,\s*255,\s*0\.98\);/);
  assert.doesNotMatch(channelsCss, /#channelModal\s+\.channel-editor-footer\s*\{[\s\S]*?position:\s*fixed;/);
  ...
  assert.match(channelsCss, /\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-url::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-exact::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-status::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-latency::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-requests::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-actions::before,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-toggle::before\s*\{[\s\S]*?content:\s*none;/);
...
test('channels 编辑弹窗手机端让 URL 精确路径说明独占一行', () => {
  assert.match(channelsHtml, /channel-editor-exact-url-hint/);
  assert.match(channelsCss, /@media\s*\(max-width:\s*768px\)\s*\{[\s\S]*?\.channel-editor-section-header--inline\s*\{[\s\S]*?flex-direction:\s*column;[\s\S]*?align-items:\s*stretch;/);
  assert.match(channelsCss, /\.channel-editor-section-header--inline\s+\.channel-editor-section-title\s*\{[\s\S]*?width:\s*100%;[\s\S]*?flex-wrap:\s*wrap;[\s\S]*?overflow:\s*visible;/);
  assert.match(channelsCss, /\.channel-editor-section-header--inline\s+\.channel-editor-exact-url-hint\s*\{[\s\S]*?flex:\s*0\s+0\s+100%;[\s\S]*?width:\s*100%;/);
  assert.match(channelsCss, /\.channel-editor-section-header--inline\s+\.channel-editor-section-actions\s*\{[\s\S]*?width:\s*100%;[\s\S]*?justify-content:\s*flex-start;/);
});

test('channels 编辑弹窗手机端 URL 状态退化为纯指示点，延迟和调用指标不再互相挤压', () => {
  assert.match(channelsCss, /\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-status\s*\{[\s\S]*?grid-column:\s*2;[\s\S]*?grid-row:\s*2;/);
  assert.match(channelsCss, /\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-latency,\s*[\r\n\s]*\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-requests\s*\{[\s\S]*?min-width:\s*0;/);
  assert.match(channelsCss, /\.inline-url-table\s+tbody\s+\.mobile-inline-row\s+td\.inline-url-col-status\s+\.inline-url-status-badge\s*\{[\s\S]*?padding:\s*0;[\s\S]*?background:\s*transparent;[\s\S]*?font-size:\s*0;/);
