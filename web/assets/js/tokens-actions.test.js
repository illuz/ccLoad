...
test('tokens 列表为当日限额调整显示独立醒目徽标', () => {
  ...
  assert.match(tokensScript, /function buildDailyLimitOverrideBadgeHtml\(token\)/);
  ...
  assert.match(tokensCss, /\.token-row-badge--daily-override\s*\{/);
});
