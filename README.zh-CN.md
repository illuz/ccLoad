...
- ⏰ **模型感知冷却**：结构化 `model_cooldown` 和上游 HTTP 5xx 都先只冷却当前实际模型，同渠道其他模型仍可用；只有所有配置模型或所有启用 Key 都在冷却时才升级为渠道冷却。渠道开启“固定模型/渠道冷却”后，失败模型和所属渠道都按配置秒数冷却，并覆盖上游返回的重置时间。
...
| 🔐 **令牌限制** | Token/分组费用、模型和渠道 allow/deny 策略 | 精细化访问控制 |
...
- **冷却管理器**（DRY原则）：
  ...
  - 结构化 `model_cooldown` 和上游 HTTP 5xx 按 `(channel_id, 实际上游模型)` 持久化，同渠道其他模型仍可选
  - 所有配置模型或所有启用 Key 均冷却时，自动升级为渠道冷却
  - 开启“固定模型/渠道冷却”后，模型级故障会让失败模型和所属渠道使用同一个固定截止时间，并覆盖更长的上游重置时间
- **多URL选择器**（URLSelector）：
...
- **渠道限制**：Token 和 Token 分组均可使用渠道允许列表（`allow`）或拒绝列表（`deny`）；列表为空时在两种模式下都表示不限制
- **分组继承**：Token 继承分组渠道限制时，渠道 ID 列表和 allow/deny 模式会一并继承
