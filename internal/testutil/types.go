...
// ImageGenerationOptions marks a non-streaming Chat Completions image test.
...
type TestChannelRequest struct {
	...
	RequestedModel    string                  `json:"-"`                            // 原始模型名；管理测试内部保留思考后缀
	...
	ImageGeneration   *ImageGenerationOptions `json:"-"`                            // 生图测试内部参数，不属于通用测试接口
}
