...
// HandleChannelEditor 聚合编辑器首次打开所需的数据，避免前端拼装多个时间点的快照。
...
func (s *Server) HandleChannelEditor(c *gin.Context) {
	...
	if s.configService != nil {
		scheduledCheckEnabled = s.configService.GetInt("channel_check_interval_hours", defaultChannelCheckIntervalHours) > 0
	}
