...
// This is the safe boundary shared by HTTP/1.1 and multiplexed HTTP/2: an
// expired generation receives no new requests while active responses drain.
...
func newUpstreamConnectionAgeTransport(base *http.Transport, maxAge time.Duration) *upstreamConnectionAgeTransport {
...
func (t *upstreamConnectionAgeTransport) retire(generation *upstreamHTTPTransportGeneration) {
	...
	// This does not interrupt active HTTP/1.1 requests or HTTP/2 streams.
...
// Close stops future rotations and closes idle connections in the current
// generation. Retired generations close when they retire or finish draining.
