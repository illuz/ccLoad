...
func migrate(ctx context.Context, db *sql.DB, dialect Dialect) error {
	...
	for _, defineTable := range tables {
		...
		if tb.Name() == "channels" {
			...
			if err := ensureChannelsBalanceQueryScript(ctx, db, dialect); err != nil {
				return fmt.Errorf("migrate channels balance_query_script: %w", err)
			}
		...
		if tb.Name() == "auth_tokens" {
			...
			if err := ensureAuthTokensDailyLimitOverride(ctx, db, dialect); err != nil {
				return fmt.Errorf("migrate auth_tokens daily limit override: %w", err)
			}
...
func initDefaultSettings(ctx context.Context, db *sql.DB, dialect Dialect) error {
	settings := []struct {
		...
		{"model_catalog_sync_interval_hours", "0", "float", "模型目录同步间隔(小时,支持小数,0=关闭网络同步,修改后重启生效)", "0"},
		{"channel_balance_refresh_interval_seconds", "300", "int", "渠道余额查询刷新间隔(秒,0=关闭,修改后重启生效)", "300"},
