	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		if s.IsSQLite() {
			channelUpsertWithIDSQL = `
					INSERT INTO channels(id, name, url, priority, rpm_limit, max_concurrency, request_delay_seconds, channel_type, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, channel_cooldown_fixed_enabled, channel_cooldown_fixed_seconds, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
					ON CONFLICT(id) DO UPDATE SET
						...
						channel_cooldown_fixed_enabled = excluded.channel_cooldown_fixed_enabled,
						channel_cooldown_fixed_seconds = excluded.channel_cooldown_fixed_seconds,
						updated_at = excluded.updated_at`
			channelUpsertByNameSQL = `
					INSERT INTO channels(name, url, priority, rpm_limit, max_concurrency, request_delay_seconds, channel_type, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, channel_cooldown_fixed_enabled, channel_cooldown_fixed_seconds, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		for _, cwk := range channels {
			...
			fixedEnabled := 0
			if config.ChannelCooldownFixedEnabled {
				fixedEnabled = 1
			}
			fixedSeconds := config.ChannelCooldownFixedSeconds
			if fixedSeconds < 1 {
				fixedSeconds = 1
			}
			...
			if useExplicitID {
				...
				_, err := channelStmtWithID.ExecContext(ctx,
					...
					config.RPMLimit, config.MaxConcurrency, config.RequestDelaySeconds, channelType, protocolTransformMode, boolToInt(config.Enabled), boolToInt(config.ScheduledCheckEnabled), config.ScheduledCheckModel, fixedEnabled, fixedSeconds, nowUnix, nowUnix)
				if err != nil {
