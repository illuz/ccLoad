...
func (s *SQLStore) CreateConfig(ctx context.Context, c *model.Config) (*model.Config, error) {
	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		if id == 0 {
			...
			INSERT INTO channels(name, url, priority, rpm_limit, max_concurrency, channel_type, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, daily_cost_limit, cost_multiplier, model_fixed_price_enabled, custom_request_rules, proxy_url, created_at, updated_at)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			`, c.Name, c.URL, c.Priority, c.RPMLimit, c.MaxConcurrency, channelType, protocolTransformMode,
				boolToInt(c.Enabled), boolToInt(c.ScheduledCheckEnabled), c.ScheduledCheckModel, c.DailyCostLimit, normalizeCostMultiplier(c.CostMultiplier), boolToInt(c.ModelFixedPriceEnabled), customRules, c.ProxyURL, nowUnix, nowUnix)
			if err != nil {
				return err
			}
			...
			if s.IsSQLite() {
				...
				INSERT INTO channels(id, name, url, priority, rpm_limit, max_concurrency, channel_type, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, daily_cost_limit, cost_multiplier, model_fixed_price_enabled, custom_request_rules, proxy_url, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *SQLStore) UpdateConfig(ctx context.Context, id int64, upd *model.Config) (*model.Config, error) {
	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `
			UPDATE channels
			SET name=?, url=?, priority=?, rpm_limit=?, max_concurrency=?, channel_type=?, protocol_transform_mode=?, enabled=?, scheduled_check_enabled=?, scheduled_check_model=?, daily_cost_limit=?, cost_multiplier=?, model_fixed_price_enabled=?, custom_request_rules=?, proxy_url=?, updated_at=?
		WHERE id=?
		`, name, url, upd.Priority, upd.RPMLimit, upd.MaxConcurrency, channelType, protocolTransformMode,
			boolToInt(upd.Enabled), boolToInt(upd.ScheduledCheckEnabled), upd.ScheduledCheckModel, upd.DailyCostLimit, normalizeCostMultiplier(upd.CostMultiplier), boolToInt(upd.ModelFixedPriceEnabled), customRules, upd.ProxyURL, updatedAtUnix, id)
		if err != nil {
			return err
		}
