...
func (s *SQLStore) ListConfigs(ctx context.Context) ([]*model.Config, error) {
	...
	query := `
			SELECT c.id, c.name, c.url, c.priority, c.rpm_limit, c.max_concurrency, c.channel_type, c.protocol_transform_mode, c.enabled,
			       c.scheduled_check_enabled, c.scheduled_check_model, c.channel_cooldown_fixed_enabled, c.channel_cooldown_fixed_seconds,
	...
	if modelName == "*" {
		...
		query = `
	            SELECT c.id, c.name, c.url, c.priority, c.rpm_limit, c.max_concurrency,
	                   c.channel_type, c.protocol_transform_mode, c.enabled, c.scheduled_check_enabled, c.scheduled_check_model, c.channel_cooldown_fixed_enabled, c.channel_cooldown_fixed_seconds,
	                   c.cooldown_until, c.cooldown_duration_ms, c.daily_cost_limit, c.cost_multiplier, c.model_fixed_price_enabled, c.custom_request_rules, c.proxy_url,
	                   SUM(CASE WHEN k.id IS NOT NULL AND k.disabled = 0 THEN 1 ELSE 0 END) as key_count,
	                   c.created_at, c.updated_at
	            FROM channels c
...
func (s *SQLStore) CreateConfig(ctx context.Context, c *model.Config) (*model.Config, error) {
	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		if id == 0 {
			...
			INSERT INTO channels(name, url, priority, rpm_limit, max_concurrency, channel_type, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, channel_cooldown_fixed_enabled, channel_cooldown_fixed_seconds, daily_cost_limit, cost_multiplier, model_fixed_price_enabled, custom_request_rules, proxy_url, created_at, updated_at)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			`, c.Name, c.URL, c.Priority, c.RPMLimit, c.MaxConcurrency, channelType, protocolTransformMode,
				boolToInt(c.Enabled), boolToInt(c.ScheduledCheckEnabled), c.ScheduledCheckModel, boolToInt(c.ChannelCooldownFixedEnabled), max(c.ChannelCooldownFixedSeconds, 1), c.DailyCostLimit, normalizeCostMultiplier(c.CostMultiplier), boolToInt(c.ModelFixedPriceEnabled), customRules, c.ProxyURL, nowUnix, nowUnix)
			if err != nil {
				return err
			}
...
func (s *SQLStore) UpdateConfig(ctx context.Context, id int64, upd *model.Config) (*model.Config, error) {
	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		`, name, url, upd.Priority, upd.RPMLimit, upd.MaxConcurrency, channelType, protocolTransformMode,
			boolToInt(upd.Enabled), boolToInt(upd.ScheduledCheckEnabled), upd.ScheduledCheckModel, boolToInt(upd.ChannelCooldownFixedEnabled), max(upd.ChannelCooldownFixedSeconds, 1), upd.DailyCostLimit, normalizeCostMultiplier(upd.CostMultiplier), boolToInt(upd.ModelFixedPriceEnabled), customRules, upd.ProxyURL, updatedAtUnix, id)
		if err != nil {
			return err
		}
