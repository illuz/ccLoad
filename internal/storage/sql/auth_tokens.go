...
func scanAuthToken(scanner interface {
	...
	var dailyLimitOverrideDayKey int
	...
	token.DailyLimitOverrideDayKey = dailyLimitOverrideDayKey
...
func (s *SQLStore) UpsertAuthTokenAllFields(ctx context.Context, token *model.AuthToken) error {
	...
	if s.IsSQLite() {
		_, err := s.db.ExecContext(ctx, `
			...
			ON CONFLICT(id) DO UPDATE SET
				...
				daily_limit_override_day_key = excluded.daily_limit_override_day_key,
	...
	_, err = s.db.ExecContext(ctx, `
		...
		ON DUPLICATE KEY UPDATE
			...
			daily_limit_override_day_key = VALUES(daily_limit_override_day_key),
